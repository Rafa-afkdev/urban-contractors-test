...
interface ConfirmCancelProjectProps {
  children: React.ReactNode;
  project: Project;
  updateProjectStatus: (projectId: string, newStatus: "COMPLETADO" | "CANCELADO") => Promise<void>;
}

export function ConfirmCancelProject({
  children,
  project,
  updateProjectStatus,
}: ConfirmCancelProjectProps) {
  const t = useTranslations("ConfirmCancelProject");

  const handleCancel = () => {
    if (project.id) {
      updateProjectStatus(project.id, "CANCELADO");
    }
  };
